...
from .playwright_extractor_ultimate import PlaywrightExtractorUltimate, run_extraction as playwright_run
from .google_maps_extractor_v2_ultimate import GoogleMapsExtractorV2Ultimate


class HybridEngineUltimate:
    """
    ...
    def __init__(self, use_cache=True, prefer_playwright=True):
        ...
        if self.use_cache:
            self.cache = CacheManagerUltimate()
        else:
            self.cache = None
    ...
    def extract_business(self, url, force_fresh=False, include_reviews=True, max_reviews=5):
        ...
        try:
            selenium_extractor = GoogleMapsExtractorV2Ultimate(headless=True, stealth_mode=True)
    ...
    async def _extract_with_playwright(self, url, include_reviews, max_reviews):
        ...
        extractor = PlaywrightExtractorUltimate(headless=True, block_resources=True, intercept_network=True)
    ...
    async def _extract_parallel_playwright(self, urls, max_concurrent):
        ...
        extractor = PlaywrightExtractorUltimate()
